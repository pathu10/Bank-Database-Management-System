...
function setBalance($amount,$process,$accountNo)
{
	...
	else
	{
		...
		return $con->query("update userAccounts set loan = '$loan' where accountNo = '$accountNo'");	
	}
}
...
function makeTransaction($action,$amount,$other)
{
	...
	if ($action == 'payloan')
	{
		return $con->query("insert into transaction (action,payloan,other,userId) values ('credit','$amount','$other','$_SESSION[userId]')");
...
function makeTransactionCashier($action,$amount,$other,$userId)
{
	...
	if ($action == 'payloan')
	{
		return $con->query("insert into transaction (action,credit,other,userId) values ('payloan','$amount','$other','$userId')");
